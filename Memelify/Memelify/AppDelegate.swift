...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    // Run local notification service periodically.
    var TimerFunc: Timer!
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        self.TimerFunc = Timer.scheduledTimer(timeInterval: 120, target: self, selector: #selector(AppDelegate.scheduleNotifications), userInfo: nil, repeats: true)
        return true
    }
    ...
    @objc func scheduleNotifications() {
        ...
        if !isEnabled {
            print("Notification setting is off")
            return
        }
        ...
        Alamofire.request(api).responseJSON { response in
            if let meme = response.result.value as? [String: Any] {
                guard let meme_url = meme["url"] else {
                    print("Notifcation remote service is not enabled. Skipping")
                    return
                }
